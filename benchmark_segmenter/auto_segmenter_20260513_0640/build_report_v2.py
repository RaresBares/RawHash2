...
9. All-segmenter Pareto: speed-up vs delta accuracy, marker = mode, color = family
"""
...
def page_accuracy(pdf):
    ...
    ax.set_ylabel("Accuracy (ovl01: F1 at >= 10% overlap)")
    ax.set_title("Accuracy: default vs rule v2 vs per-dataset oracle")
...
def page_accgain(pdf):
    ...
    ax.set_xticks(x); ax.set_xticklabels([label(d) for d in DATASETS], fontsize=8)
    ax.set_ylabel("delta accuracy (ovl01 pt)")
    ax.set_title("Accuracy delta: rule v2 - default (positive = better)")
...
def page_pareto(pdf):
    ...
    ax.set_ylabel("delta accuracy vs default (ovl01 pt)")
...
def main():
    ...
    with open(csv_path, "w") as fh:
        fh.write("ds,chem,ref_mb,gc,default_acc,default_wall,"
                 "rule_seg,rule_mode,rule_boost,rule_acc,rule_wall,"
                 "oracle_seg,oracle_mode,oracle_boost,oracle_acc,oracle_wall,"
                 "speedup,delta_acc_pt,wall_spread_pct\n")
        for c in CHOICES:
            d = c["default"]; p = c["rule"]; o = c["oracle"]
            sp = (d["wall_median"]/p["wall_median"]) if (p and p["wall_median"]) else 0
            da = (p["ovl01_median"] - d["ovl01_median"])*100 if (p and d) else 0
            spread = (p["wall_max"]/p["wall_min"] - 1)*100 if (p and p["wall_min"]) else 0
            fh.write(f'{c["ds"]},{c["chem"]},{c["ref_mb"]:.3f},{c["gc"]:.2f},'
                     f'{d["ovl01_median"]:.4f},{d["wall_median"]:.2f},'
                     f'{c["rule_seg"]},{c["rule_mode"]},{c["rule_boost"] or 1.0:.1f},'
                     f'{p["ovl01_median"]:.4f},{p["wall_median"]:.2f},'
                     f'{o["seg"]},{o["mode"]},{o["boost"] or 1.0:.1f},'
                     f'{o["ovl01_median"]:.4f},{o["wall_median"]:.2f},'
                     f'{sp:.3f},{da:+.2f},{spread:.1f}\n')
    print(f"wrote {out}")
