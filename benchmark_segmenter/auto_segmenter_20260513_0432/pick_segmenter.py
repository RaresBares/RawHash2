...
def pick(chemistry: str, ref_mb: float, gc_pct: float | None = None) -> tuple[str, str]:
    ...
    if chem == "R9.4":
        ...
        return ("hmm", "static")
    if chem == "R10.4":
        # D6 (E. coli R10.4, large fast5): binseg with Auto-K matches PELT's
        # accuracy (within 0.1 pt) while running 9x faster than PELT and
        # marginally faster than default.
        return ("binseg", "auto")
    if chem == "R10.4.1":
        if ref_mb < 10:
